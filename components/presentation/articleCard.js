...
export function ArticleCard({ title, description, image, date, tags, likes }) {
  ...
  return (
    <div className="card w-72 md:w-96 shadow-lg shadow-primary">
      ...
      <h3 className="card-title bg-primary py-3 px-4 text-2xl uppercase md:text-3xl">
        {title}
      </h3>
      <div className="card-body max-h-64 py-3">
        <p className="font-light text-sm md:text-base">{date}</p>
        <p className="text-lg overflow-ellipsis overflow-hidden md:text-xl">
          {description}
        </p>
      </div>
      <div className="card-actions justify-end my-1 px-4 md:text-xl">
      ...
      <div className="card-actions justify-end my-5 px-4">
        ...
        {tags.map((tag, index) => (
          <div
            className={`badge badge-${badgeStyles[index]} px-2 py-2 md:text-lg md:py-4`}
            key={tag}
          >
            {tag}
          </div>
